...
import org.interfaces.lambda.CompareWith;
import org.interfaces.lambda.Evaluate;
import org.interfaces.lambda.MathOperation;
...
public class TestInterfaces {
  ...
  public void testCountLength() {
    ...
    // Map<4, 6>
    // sorted by key
    Arrays.stream(keepOnlyWords(txt).split("\\s"))
        .map(String::length)
  ...
  public void testCountChars() {
    countCharFrequency(txt).entrySet()
        .forEach(System.out::println);
  }

  @Test
  public void testInterface() {
    CompareWith<Integer, Integer> moreThan = (n, m) -> n > m;
    System.out.println(moreThan.compareWith(6, 11));

    CompareWith<Character, Character> afterChar = (c, b) -> c > b;
    System.out.println(afterChar.compareWith('A', 'K'));
  }

  @Test
  public void testSum() {
    MathOperation<Integer> addTwoInts = Integer::sum;
    System.out.println(addTwoInts.sum(3, 5));

    MathOperation<Float> addTwoFloats = Float::sum;
    System.out.println(addTwoFloats.sum(5.5F, 6.5F));

    BiPredicate<String, Integer> expectedLength = (s, n) -> s.length() == n;
    System.out.println(expectedLength.negate().test("Hello", 5));
    System.out.println(expectedLength.test("World", 10));
  }

  @Test
  public void testSupplier() {
    Supplier<String> addPhrase = () -> "Add number";
    System.out.println(addPhrase.get());
  }

  @Test
  public void testConsumer() {
    Consumer<String> prntStr = s -> {
      System.out.println(s + " from Consumer");
    };

    prntStr.accept("Barmalei");
  }

  @Test
  public void testBiConsumer() {
    BiConsumer<Integer, String> repeatTimes = (n, s) -> System.out.println(s.repeat(n));
    repeatTimes.accept(3, "lala");
  }

  @Test
  public void testBiFunction() {
    BiFunction<String, Integer, String> repeatNTimes = String::repeat;
    System.out.println(repeatNTimes.apply("Aza ", 3));
  }


  // clear text and leave just words in the lower case
  ...
  private Map<Character, Long> countCharFrequency(String text) {
    if (text == null || text.isEmpty()) {
      return new HashMap<>();
    }

    return (Map<Character, Long>) text.chars()
        .mapToObj(c -> (char) c)
        .collect(Collectors.groupingBy(
            Function.identity(),
            Collectors.counting()
